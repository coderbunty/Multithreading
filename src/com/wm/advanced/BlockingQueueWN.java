...
/*
  ...
  Eg. used in ExecutorService thread pool (LinkedBlockingQueue<Runnable>)
...
public class BlockingQueueWN<T extends Comparable<T>> {
  ...
  public synchronized void insert(T data) throws InterruptedException {
    ...
    /* using while loop (instead of IF condition) with condition to wait() is a best practice that is followed as Java doesn't guarantee that a thread will be woken up by only notify() or notifyAll()
